...
const getFinalBinding = (
	...
	if (
		exportName.length > 0 &&
		exportsInfo.getUsed(exportName, runtime) === UsageState.Unused
	) {
		return {
			info,
...
class ConcatenatedModule extends Module {
	...
	codeGeneration({
		...
		for (const info of modulesWithInfo) {
			...
			switch (info.type) {
				case "concatenated": {
					...
					const namespaceObjectName =
						info.internalNames.get("__WEBPACK_NAMESPACE_OBJECT__") ||
						this.findNewName(
							"namespaceObject",
							allUsedNames,
		...
		for (const [name, symbol] of rootInfo.exportMap) {
			...
			if (used) {
				exportsMap.set(
					used,
					() => `/* binding */ ${rootInfo.internalNames.get(symbol)}`
				);
			} else {
				unusedExports.add(name || "namespace");
			}
		}
		for (const [name, { module, exportName }] of rootInfo.reexportMap) {
			const used = /** @type {string | false } */ (moduleGraph
				.getExportsInfo(this.rootModule)
				.getUsedName(name, runtime));
			if (used) {
				if (!exportsMap.has(used)) {
					const info = moduleToInfoMap.get(module);
					if (info) {
						...
						exportsMap.set(
							used,
							() => `/* reexport (missing module) */ undefined`
						);
					}
					unusedExports.delete(name);
				}
			} else {
	...
	findNewName(oldName, usedNamed1, usedNamed2, extraInfo) {
		...
		if (
			...
			name === "_unused_webpack_default_export"
		) {
			name = "";
		}
		if (name === "__WEBPACK_NAMESPACE_OBJECT__") name = "namespaceObject";
