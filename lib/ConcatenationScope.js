...
class ConcatenationScope {
	...
	registerReexport(exportName, module, referencedExportName) {
		if (!this._currentModule.reexportMap.has(exportName)) {
			this._currentModule.reexportMap.set(exportName, {
				module,
				exportName: referencedExportName
			});
		} else {
			console.log(
				"registerReexport conflict",
				exportName,
				this._currentModule.reexportMap.get(exportName).module.identifier(),
				module.identifier()
			);
		}
	}
